...
def optimize(max_emissions, options):
    ...
    for d in range(number_days):
        for t in range(timesteps):
            for i in range(weight_days[d]):
                ...
                SunRad_part[:, d, t] += (
                            SunRad[:, i * timesteps + t + monthsCum0[d] * timesteps] / weight_days[d] / 1000)  # kW
    ...
    price = {"CHP": 0.0541,  # Subsidy for electricity produced by CHP units from German co-generation act in EUR/kWh
             ...
             ("CHP", "sell"): np.loadtxt("input_data/remuneration_chp.txt"),
    ...
    # CHP units
    inv["CHP"] = [13200, 15500, 17685, 21905,
                  32140]  # Specific investment costs in Euro/kW for chp units 1, 2, 3, 5, 10 kW electrical
    ...
    eta[
        "PV"] = 0.14 * 0.85  # Efficiency of the photovoltaic cells (0.14) and further losses (0.85 -> converter, wiring...)
    ...
    cop = (
                      a_cop * t_amb + b_cop) / t_flow  # Coefficient of performance of HP dependent on ambient temperature and t_flow
    ...
    model.addConstr(c_inv[dev] == crf * (1 - rval[dev][0]) * tax * sum(x[dev, i] * inv[dev][i]
                                                                       for i in range(
        number_devices[dev])) - 1 / t_clc * subsidy)
    ...
    for d in range(number_days):
        ...
        for t in range(timesteps):
            ...
            model.addConstr(
                storage["TES", d, t] <= rho_water * cp_water * (deltaT["HP"] - deltaT["TES"]) * s_max["TES", d, t] +
            ...
            for i in range(number_devices["CHP"]):
                ...
                if cap["CHP"][i] < 5:  # According to Spieker and Tsatsaronis
                    ...
                    model.addConstr(q_max["CHP", d, t, i] == 1.0 / params["CHP", "beta"] *
                                    (p_max["CHP", d, t, i] - y["CHP", d, t, i] * (
                                                params["CHP", "alpha"] * q_CHP[i] + params["CHP", "gamma"])))
            ...
            for i in range(number_devices["Boiler"]):
                ...
                model.addConstr(
                    q_max["Boiler", d, t, i] >= y["Boiler", d, t, i] * cap["Boiler"][i] * part_load["Boiler"][i])
    ...
    if model.status == gp.GRB.OPTIMAL or model.status == gp.GRB.TIME_LIMIT:
        ...
        for dev in ["EH", ]:
            if x[dev].X == 1:
                print
                dev, "=", cap_design[dev].X, u"[kW]"
        if x["PV"].X == 1:
            print
            "PV_area", "=", area["PV"].X, "[m2]"
            print
            "PV_cap", "=", cap_design["PV"].X, "[kW]"
        if x["STC"].X == 1:
            print
            "STC", "=", area["STC"].X, "[m2]"
