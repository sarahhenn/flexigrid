...
@author: she
"""
#Idee: Button anzeigen lassen zum auswählen offener Optionen wie ob der Heißwasserverbrauch elektrisch ist oder nicht?
...
from python.simple_plot_bat import simple_plot_bat
...
operationFolder = "C:\\users\\flori\\pycharmprojects\\Bachelorarbeit\\flexigrid"
    ...
    for d in days:
        for t in timesteps:
            if n in nodes["trafo"]:
                ...
                model.addConstr(powerLine.sum(n, '*', d, t) - powerLine.sum('*', n, d, t) ==
                                powerPV[n, d, t] - powerPlug[n, d, t] - powerCh[n, d, t] + powerDis[n, d, t],
                                name="node balance_" + str(n))
...
for [n, m] in nodeLines:
    for d in days:
        for t in timesteps:
            model.addConstr(powerLine[n, m, d, t] <= powerLine_max[n, m],
                            name="line power max_" + str(n) + str(m) + str(t))
            model.addConstr(powerLine[n, m, d, t] >= (-1) * powerLine_max[n, m],
                            name="line power min_" + str(n) + str(m) + str(t))
    ...
    for d in days:
        for t in timesteps:
            ...
            model.addConstr(SOC[n, d, t] == (SOC_previous
                                             + (dt * (
                                powerCh[n, d, t] * batData["etaCh"] - powerDis[n, d, t] / batData["etaDis"])) / batData[
                                                 "energyContent"])
...
model.setObjective(sum(sum(powerTrafo[d, t] * clustered["co2"][d, t] for t in timesteps) for d in days),
                   gp.GRB.MINIMIZE)
...
print("wir sind am Ende angekommen")
print(model)

print("kurzer Zwischenstopp")
