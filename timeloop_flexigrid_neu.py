...
def timeseries_each_day(output_dir, net, timesteps, d, powInjRet, powSubtrRet, gridnodes):
    ...
    profilesInj, profilesSubtr, dsInj, dsSubtr, dsTotal = retrieve_data_source(timesteps, d, powInjRet, powSubtrRet, gridnodes)
    ...
    const_load, const_gen = create_controllers(net,dsInj,dsSubtr,dsTotal,gridnodes)
    ...
    timeseries.run_timeseries(net, time_steps, output_writer=ow, continue_on_divergence=True)
...
def retrieve_data_source(timesteps, d, powInjRet, powSubtrRet, gridnodes):
    ...
    dsInj = timeseries.DataFrame(profilesInj)
    dsSubtr = timeseries.DataFrame(profilesSubtr)
    dsTotal = timeseries.DataFrame(profilesTotal)
...
def create_controllers(net, dsInj, dsSubtr, dsTotal,gridnodes):
    ...
    const_load = timeseries.ConstControl(net, element= 'load', variable= 'p_mw', element_index=net.load.index, data_source=dsSubtr, profile_name=gridnodes_index)
    const_gen = timeseries.ConstControl(net, element= 'sgen', variable= 'p_mw', element_index=net.sgen.index, data_source=dsInj, profile_name=gridnodes_index)

    return const_load, const_gen
...
def create_output_writer(net, timesteps, output_dir):
    time_steps = timesteps
    ow = timeseries.OutputWriter(net, time_steps, output_path=output_dir, output_file_type=".json")
